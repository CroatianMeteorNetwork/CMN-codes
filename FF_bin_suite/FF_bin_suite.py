...
def buildFF(ff, kframe):
	"""Function for returning the K frame from a FF bin file
...
def makeGIF(FF_input, start_frame=0, end_frame =255, ff_dir = '.', deinterlace = True, print_name = True, optimize = True, Flat_frame = None, Flat_frame_scalar = None, dark_frame = None, gif_name_parse = None, repeat = True, fps = 25, minv = None, gamma = None, maxv = None, perfield = False):
	...
	for entry in FF_input:
		...
		for k in range (start_frame, end_frame+1):
			img_array = buildFF(readFF(FF_file), k)
...
def process_array(img_array, Flat_frame = None, Flat_frame_scalar = None, dark_frame = None, deinterlace = False, frame = 0):
	...
	if (dark_frame != None) or (Flat_frame != None):
		img_array = np.clip(img_array, 0, 255)

	if frame == 1: #Odd frame:
...
def get_processed_frames(ff_bin, Flat_frame, Flat_frame_scalar, start_frame = 0, end_frame = 255):
	""" Makes calibrated BMPs of a particular detection, without average frame background.
	ff_bin: name and path to the FF*.bin file
	Flat_frame: flat frame array (load flat frame or make it)
	Flat_frame_scalar: flat frame median value (load flat frame or make it)
	start_frame:
	end_frame:
	...
	img_average = readFF(ff_bin).avepixel
	img_average = img_average.astype(np.float)
	Flat_frame[Flat_frame == 0] = 1

	ave_noflat = img_average / Flat_frame
	ave_noflat = np.multiply(ave_noflat, Flat_frame_scalar)
	ave_noflat = np.clip(ave_noflat, 0, 255)

	for nframe in range(start_frame, end_frame+1):

		frame_img = buildFF(readFF(ff_bin), nframe) #Get individual frames
		frame_img = frame_img.astype(np.float)

		frame_noflat = frame_img / Flat_frame
		frame_noflat = np.multiply(frame_noflat, Flat_frame_scalar)
		frame_noflat = np.clip(frame_noflat, 0, 255)

		frame_sub_img = np.subtract(frame_noflat, ave_noflat) #Substract average from frame image

		deinterlaced_frame = deinterlace_blend(frame_sub_img) #Deinterlace only

		saveImage(deinterlaced_frame, ff_bin+"frame_"+str(nframe)+"_individual.bmp", print_name = False)
...
def make_flat_frame(flat_dir, flat_save = 'flat.bmp', col_corrected = False, dark_frame = None):
	...
	flat_raw = [flat_dir+line for line in os.listdir(flat_dir) if line.split('.')[-1] == 'bin']
	flat_arrays = []
	...
	try:
		...
		return False
	nrows = int(readFF(first_raw).nrows[0])
	ncols = int(readFF(first_raw).ncols[0])
...
def get_FTPdetect_coordinates(FTPdetect_file, ff_bin):
	""" Returns a list of FF*.bin coordinates of a specific bin file as a list of tuples e.g. [(15, 20), (16, 21), (17, 22)] and the rotation angle of the meteor"""
	...
	for line in FTPdetect_file_content[12:]:
		...
		if found_bin == True:
			...
			coord_list.append((int(round(float(line[2]), 0)), int(round(float(line[1]), 0))))
...
#get_processed_frames(img_dir+img_name, Flat_frame, Flat_frame_scalar, 150, 155)
